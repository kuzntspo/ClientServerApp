...
public class EditQuoteConroller {
    ...
    void save(MouseEvent event) throws SQLException, ClassNotFoundException {
        ...
        if (quote.isEmpty()||name.isEmpty()||surname.isEmpty()||middle_name.isEmpty()||subject.isEmpty()||date.isEmpty()){
            ...
            alert.setHeaderText(null);
            alert.setContentText("Ваша цитата изменена. Обновите таблицу");
            alert.showAndWait();
        }
    }
    ...
    private void getQuery() {
        query = "UPDATE `teacher_quotes` SET `user_id`=?,`quote`=?,`surname`=?,`name`=?,`middle_name`=?,`subject`=?,`date`=? WHERE id = " + TableController.currentId;
    }
